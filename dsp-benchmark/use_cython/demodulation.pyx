...
cdef void _convolve(double* pdata1, const int len1, double* pdata2, const int len2, const int bufsize, double* presult) nogil:
	...
	with nogil, parallel():
		...
		for j in prange(N, schedule ='guided'):
			for i in range(nreference):
				...
				_convolve(yref, nsignal, &window[0], nwindow, bufsize, conv)
...
@cython.nonecheck(False)
@cython.cdivision(True)
cpdef void ipp_sk(double[:,::1] signal, double[:,::1] reference, double[::1] window,
			 double[:,::1] traces = None,
			 double[:,::1] averages = None,
			 double[:,::1] statistics = None):
	""" Demodulation of periodic data
